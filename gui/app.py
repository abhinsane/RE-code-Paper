...
from pq_evoting.config import BIO_MATCH_THRESHOLD, AUTH_SESSION_TIMEOUT
...
tab_setup, tab_reg, tab_vote, tab_results, tab_chain, tab_revoke = st.tabs([
    "🏛️ Setup",
    "📋 Register",
    "🗳️ Vote",
    "📊 Results",
...
with tab_reg:
    ...
    if S.phase == "setup":
        ...
        st.warning("🔒 Registration is locked — voting is in progress. No new voters can be added.")
        ...
        with col1:
            ...
            if S.auth_result is not None:
                ...
                if not ar["ok"]:
                    ...
                    if time.time() - ar.get("timestamp", 0) > AUTH_SESSION_TIMEOUT:
                        st.error(
                            f"⏰ **Authentication session expired** — your biometric "
                            f"session exceeded the {AUTH_SESSION_TIMEOUT // 60}-minute "
                            f"window. Please authenticate again."
                        )
                        S.auth_result = None
                        st.rerun()
...
# ════════
# TAB 6 — REVOKE BIOMETRIC
# ════════

with tab_revoke:
    st.header("🔑 Revoke / Re-enrol Biometric Template")
    st.caption(
        "Use this if your token (PIN) has been compromised or you want to switch to a new PIN. "
        "The old template is cryptographically destroyed and a fresh one is created with the new token."
    )
    ...
    if S.phase == "setup":
        ...
        with st.form("revoke_form"):
            rev_voter = st.selectbox(
                "Voter ID",
                options=list(S.voters.keys()),
                help="Select the voter whose template you want to revoke.",
            )
            ...
            with col_old:
                old_pin = st.text_input(
                    ...
                    type="password",
                    value="secret123",
                    help="The PIN used during original registration.",
                )
            with col_new:
                new_pin = st.text_input(
                    ...
                    type="password",
                    value="",
                    help="The replacement PIN. Must be different from the current one.",
                )
            rev_reason = st.text_input(
                "Reason (optional)",
                value="",
                placeholder="e.g. token compromised, device lost",
            )
            fp_rev_mode = st.radio(
                "Fingerprint",
                ["Use enrolled image", "Upload new image"],
                horizontal=True,
                help="The fingerprint must match the one enrolled under the current PIN.",
            )
            uploaded_rev_fp = None
            if fp_rev_mode == "Upload new image":
                uploaded_rev_fp = st.file_uploader(
                    "Fingerprint image",
                    type=["bmp", "png", "jpg", "jpeg"],
                    key="rev_fp",
                )
            revoke_btn = st.form_submit_button(
                "🔑 Revoke & Re-enrol", type="primary", use_container_width=True
            )

        if revoke_btn and rev_voter:
            if not new_pin:
                st.error("New PIN cannot be empty.")
            elif old_pin == new_pin:
                st.error("New PIN must differ from the current PIN.")
            else:
                voter_obj = S.voters.get(rev_voter)
                if voter_obj is None:
                    st.error("Voter object not found in session. Re-register first.")
                else:
                    if fp_rev_mode == "Upload new image" and uploaded_rev_fp:
                        rev_fp_path = _save_upload(uploaded_rev_fp, f"rev_{rev_voter}")
                    else:
                        rev_fp_path = S.voter_fps.get(rev_voter, {}).get("enrol", "")
                    ...
                    try:
                        ...
                        if ok:
                            ...
                            st.error("Revocation failed — voter not found or signature invalid.")
                    except ValueError as exc:
                        st.error(f"Biometric verification failed: {exc}")
