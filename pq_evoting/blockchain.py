...
class VoteRecord:
    ...
    signature:   str    # ML-DSA-65 signature of (enc_vote ‖ zkp_hash ‖ bio_commitment)
    voter_sig_pk:   str  # Voter's ML-DSA-65 public key (hex)
    bio_commitment:  str   # SHA3-256 of live BioHash at cast time (hex) — needed to
                           # reconstruct the sig payload during verify_chain()
        ...
        return json.dumps(
            {
                ...
                "bio_commitment": self.bio_commitment,
    ...
    def verify_chain(self) -> bool:
        ...
        * prev_hash correctly references the predecessor's hash
        ...
        for i in range(1, len(self.chain)):
            ...
            # FIX: verify each voter's ML-DSA-65 signature over their own
            # VoteRecord.  Previously the blockchain only verified the
            # authority's block-header signature, so an attacker with DB
            # access could swap out vote records, recompute the Merkle root,
            # and re-sign the header — the voter's signature provided no
            # protection.  Now every voter signature is validated inline.
            # sig_payload format must match voting_system.py cast_vote/receive_vote.
            for vote in cur.votes:
                try:
                    voter_pk    = bytes.fromhex(vote.voter_sig_pk)
                    sig_bytes   = bytes.fromhex(vote.signature)
                    # Payload must match cast_vote() in voting_system.py:
                    # enc_hex | zkp_proof_hash | bio_commitment_hex
                    sig_payload = (
                        vote.encrypted_vote + "|"
                        + vote.zkp_proof_hash + "|"
                        + vote.bio_commitment
                    ).encode()
                    if not pq_verify(voter_pk, sig_payload, sig_bytes):
                        return False
                except Exception:
                    return False
