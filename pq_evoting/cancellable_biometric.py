...
def _template_sign_payload(encrypted_template: dict, token_hash: str) -> bytes:
    """
    Canonical byte string that the authority signs over at enrolment and
    verifies at authentication.

    Covers every security-relevant field of the stored record:
      - kem_ct  : the ML-KEM-768 ciphertext (if swapped → sig fails)
      - aes_ct  : the AES-GCM ciphertext    (if swapped → sig fails)
      - nonce   : the GCM nonce             (if swapped → sig fails)
      - token_hash: links template to this voter's token

    All fields are hex strings (as returned by pq_encrypt), so
    concatenation is unambiguous and deterministic.
    """
    return (
        encrypted_template["kem_ct"]
        + encrypted_template["aes_ct"]
        + encrypted_template["nonce"]
        + token_hash
    ).encode()
...
class CancellableBiometric:
    ...
    def _compute_hog(self, img: np.ndarray) -> np.ndarray:
        ...
        feat = feat.flatten().astype(np.float32) # type: ignore
    ...
    def compute_biohash(
        ...
        # FIX: reject mismatched feature vectors instead of silently padding or
        # truncating.  Silent padding fills missing dimensions with zeros, which
        # can cause two different fingerprints to produce colliding BioHash
        # templates in the projection space, enabling biometric impersonation.
        f = np.asarray(feature, dtype=np.float32)
        if len(f) != self._raw_dim:
            raise ValueError(
                f"Feature vector length {len(f)} does not match expected "
                f"{self._raw_dim}.  Ensure the fingerprint image is a valid "
                f"{self._IMG_SIZE}×{self._IMG_SIZE} grayscale image."
            )
    ...
    def enroll(
        ...
        BioHash template, PQ-encrypt it, and sign the ciphertext so that
        any later substitution is detectable.
        ...
        authority_kem_pk : Election authority's ML-KEM-768 public key — encrypts
                           the template.
        authority_sig_sk : Election authority's ML-DSA-65 signing key — signs
                           the encrypted template to prevent template substitution
                           attacks.  An adversary who swaps the ciphertext in the
                           database cannot forge this signature without the secret
                           signing key.

        Returns
        -------
        dict with keys:
            encrypted_template – PQ-encrypted BioHash ciphertext (dict)
            template_hash   – SHA3-256 of raw BioHash (hex)
            token_hash   – SHA3-256 of user_token (hex)
            template_sig  – ML-DSA-65 signature over the ciphertext (hex)
            feature_dim  – int
        """
        ...
        # FIX: sign the encrypted template with the authority's ML-DSA-65 key.
        # This closes the template substitution attack: an adversary who has
        # write access to the voter registry and knows the authority's public
        # KEM key could otherwise replace a legitimate ciphertext with an
        # encryption of their own BioHash and then authenticate as that voter.
        # With this signature, any modification of the stored ciphertext is
        # detected during verify() before the KEM secret key is ever used.
        signing_payload = _template_sign_payload(encrypted_template, token_hash)
        template_sig    = pq_sign(authority_sig_sk, signing_payload)
    ...
    def compute_bio_commitment(self, image_path: str, user_token: bytes) -> bytes:
        """
        Compute a cryptographic commitment to the live BioHash.

        This can be called by the voter independently (no authority keys
        needed) to derive the same value the authority stores at auth time.
        The commitment is SHA3-256 of the raw BioHash bytes, binding the
        voter's biometric to any cryptographic context that includes it.

        Returns
        -------
        32-byte SHA3-256 digest of the live BioHash.
        """
        feature = self.extract_features(image_path)
        biohash = self.compute_biohash(feature, user_token)
        return hashlib.sha3_256(biohash.tobytes()).digest()
    ...
    def verify(
        ...
        2. Verify the ML-DSA-65 signature over the stored ciphertext.
           Rejects any template whose ciphertext was substituted after enrolment.
        3. Decrypt the stored BioHash template using the authority's secret key.
        4. Compute BioHash of the live fingerprint.
        5. Compute fractional Hamming similarity; accept if ≥ threshold.

        Parameters
        ----------
        authority_sig_pk : Authority's ML-DSA-65 public key for template
                           integrity verification.  If omitted (empty bytes),
                           integrity checking is skipped — only use this for
                           legacy templates enrolled before the fix was applied.

        Returns
        -------
        (is_match: bool, similarity_score: float, bio_commitment: bytes)

        bio_commitment is SHA3-256 of the live BioHash — a cryptographic
        commitment that can be embedded in the ZKP context and ballot
        signature to bind the biometric to the ballot.  Returns 32 zero
        bytes on any check failure.
        """
        null = b"\x00" * 32
        ...
        # 2. Template integrity check (template substitution attack prevention)
        if authority_sig_pk:
            raw_sig = enrolled.get("template_sig", "")
            if not raw_sig:
                # Template was enrolled before the integrity fix — reject.
                # Re-enrolment with enroll() is required.
                return False, 0.0, null
            signing_payload = _template_sign_payload(
                enrolled["encrypted_template"], enrolled["token_hash"]
            )
            if not pq_verify(authority_sig_pk, signing_payload, bytes.fromhex(raw_sig)):
                # Signature mismatch → stored ciphertext was tampered with.
                return False, 0.0, null
        ...
        # Bio-commitment: SHA3-256 of the live BioHash bytes
        bio_commitment = hashlib.sha3_256(query.tobytes()).digest()
    ...
    def cancel_and_reenroll(
        ...
        authority_kem_sk: bytes,
        enrolled: dict,
        authority_sig_sk: bytes = b"",
        authority_sig_pk: bytes = b"",
    ) -> dict:
        """
        Revoke a compromised template and create a fresh one with *new_token*.

        The old token is verified first (including template integrity if
        authority_sig_pk is provided); if it does not match the live fingerprint
        ...
        """
        is_match, score, _ = self.verify(
            image_path, old_token, authority_kem_sk, enrolled,
            authority_sig_pk=authority_sig_pk,
        )
        if not is_match:
            raise ValueError(
...
def create_synthetic_fingerprint(output_path: str, seed: int = 42) -> None:
    """
    Generate a deterministic synthetic fingerprint-like greyscale BMP image.

    Uses Perlin-style sinusoidal ridge patterns seeded by *seed* so different
    seeds produce meaningfully different images (for impostor-rejection tests).
    Writes a 256×256 8-bit greyscale BMP to *output_path*.
    """
    rng = np.random.default_rng(seed)
    size = 256
    x, y = np.meshgrid(np.linspace(0, 4 * np.pi, size), np.linspace(0, 4 * np.pi, size))

    # Random ridge orientation and frequency per seed
    angle  = rng.uniform(0, np.pi)
    freq  = rng.uniform(0.8, 1.4)
    phase   = rng.uniform(0, np.pi)
    noise  = rng.standard_normal((size, size)) * 0.15

    ridges  = np.sin(freq * (x * np.cos(angle) + y * np.sin(angle)) + phase) + noise
    ridges  = ((ridges - ridges.min()) / (ridges.max() - ridges.min()) * 255).astype(np.uint8)
