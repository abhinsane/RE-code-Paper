...
class VoterRegistration:
    ...
    bio_commitment:     bytes         = field(default=b"")  # SHA3-256 of live BioHash at auth time
...
class VoterRegistry:
    ...
    def __init__(self, db_path: str = ":memory:") -> None:
        ...
        # FIX: re-entrant lock for TOCTOU protection.  Without it, two
        # concurrent calls (e.g. authenticated_not_voted() + mark_voted())
        # can race — both see has_voted==False and both accept the same voter.
    ...
    def _init_schema(self) -> None:
        self._conn.executescript("""
            CREATE TABLE IF NOT EXISTS voters (
                ...
                bio_commitment  TEXT NOT NULL DEFAULT ''
            );
    ...
    def _row_to_reg(row: sqlite3.Row) -> VoterRegistration:
        return VoterRegistration(
            ...
            bio_commitment  = bytes.fromhex(row["bio_commitment"]) if row["bio_commitment"] else b"",
        )
    ...
    def _persist(self, reg: VoterRegistration) -> None:
        ...
        self._conn.execute(
            ...
            (
                ...
                reg.bio_commitment.hex() if reg.bio_commitment else "",
            ),
        )
        self._conn.commit()
    ...
    def update_biometric_data(
        self, voter_id: str, new_bio_data: dict, reason: str = ""
    ) -> bool:
        """
        ...
        *reason* is a free-text string logged to the audit trail so that
        every cancellation has a traceable justification.
        ...
        self._persist(reg)
        event = "biometric_cancelled_and_reenrolled"
        if reason:
            # Truncate to 200 chars to keep the audit DB tidy.
            event = f"{event}: {reason[:200]}"
    ...
    def record_failed_auth(self, voter_id: str) -> bool:
        ...
        if reg.bio_fail_count >= BIO_MAX_AUTH_ATTEMPTS:
            # FIX: use time.monotonic() so an NTP clock step-back cannot
            # silently extend the lockout window or invalidate it early.
            reg.bio_locked_until = time.monotonic() + BIO_LOCKOUT_SECONDS
            self._persist(reg)
    ...
    def is_auth_locked(self, voter_id: str) -> bool:
        ...
        if reg is None or reg.bio_locked_until is None:
            return False
        # FIX: use time.monotonic() to match how lockout timestamps are set
        # in record_failed_auth — prevents clock-manipulation bypass.
        if time.monotonic() >= reg.bio_locked_until:
    ...
    def mark_authenticated(self, voter_id: str, bio_commitment: bytes = b"") -> bool:
        """
        ...
        Parameters
        ----------
        bio_commitment : SHA3-256 of the live BioHash computed during verify().
            Stored so receive_vote() can cross-check that the ballot's embedded
            bio_commitment matches the one produced at authentication time —
            cryptographically binding the biometric to the ballot.
        """
        reg = self._cache.get(voter_id)
        if reg is None or not reg.is_active:
            return False
        reg.bio_authenticated = True
        # FIX: store monotonic time so the session-expiry check in
        # is_authenticated() is immune to system clock manipulation.
        # Note: monotonic timestamps are process-local; after a restart all
        # sessions expire automatically (no old token can survive a reboot).
        reg.bio_auth_time   = time.monotonic()
        reg.bio_commitment  = bio_commitment
        self._persist(reg)
        return True
    ...
    def is_authenticated(self, voter_id: str) -> bool:
        ...
        if not (reg and reg.bio_authenticated and reg.bio_auth_time is not None):
            return False
        # FIX: use time.monotonic() — matches mark_authenticated() and prevents
        # NTP step-back from keeping an expired session token alive.
        elapsed = time.monotonic() - reg.bio_auth_time
    ...
    def get_bio_commitment(self, voter_id: str) -> bytes:
        """Return the bio_commitment stored at authentication time, or empty bytes."""
        reg = self._cache.get(voter_id)
        return reg.bio_commitment if reg else b""

    def clear_authentication(self, voter_id: str) -> None:
        """Clear the biometric auth flag, timestamp, and commitment (called after vote is cast)."""
        reg = self._cache.get(voter_id)
        if reg:
            reg.bio_authenticated = False
            reg.bio_auth_time  = None
            reg.bio_commitment = b""
            self._persist(reg)
    ...
    def mark_voted(self, voter_id: str) -> bool:
        ...
        already voted.
        """
        # FIX: hold the registry lock so check-and-mark is atomic.
        # This closes the TOCTOU race where two concurrent calls both see
        # has_voted==False and both succeed, enabling double-voting.
        with self._lock:
            reg = self._cache.get(voter_id)
    ...
    def sweep_expired_sessions(self) -> int:
        ...
        """
        # FIX: use monotonic clock to match mark_authenticated().
        now   = time.monotonic()
        count = 0
        with self._lock:
    ...
    def authenticated_not_voted(self) -> List[str]:
        ...
        """
        # FIX: hold the lock so the list snapshot is consistent — without it,
        # a concurrent mark_voted() call could mark a voter as voted between
        # the time this method returns and the caller clears the auth token,
        # leading to the voter being incorrectly flagged as "authenticated but
        # never voted" and having their auth cleared unnecessarily.
        # FIX: use monotonic clock to match mark_authenticated().
        with self._lock:
            now = time.monotonic()
            return [
