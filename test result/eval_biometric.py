...
Usage:
    python "test result/eval_biometric.py"                                        # synthetic fingerprints
    python "test result/eval_biometric.py" --dataset ./SOCOFing                   # real SOCOFing dataset
    python "test result/eval_biometric.py" --dataset ./SOCOFing --subjects 100 --plot
"""
...
def generate_synthetic_subjects(n_subjects: int = 30,
                                  n_impressions: int = 4,
                                  tmp_dir: str | None = None) -> dict:
    """
    Create n_subjects × n_impressions synthetic fingerprint PNG files.

    Different impressions for the same subject use nearby RNG seeds so
    the ridge patterns are related but not identical, mimicking real
    intra-subject variability.
    """
    if tmp_dir is None:
        tmp_dir = tempfile.mkdtemp(prefix="bio_eval_")
    subjects: dict[str, list[str]] = {}
    for s in range(n_subjects):
        paths = []
        for imp in range(n_impressions):
            path = os.path.join(tmp_dir, f"subj{s:03d}_imp{imp}.png")
            seed = s * 100 + imp          # nearby seeds → similar but not equal
            create_synthetic_fingerprint(path, seed=seed)
            paths.append(path)
        subjects[str(s)] = paths
    return subjects
...
def main() -> None:
    ...
    parser.add_argument("--dataset", default=None,
                        help="Path to SOCOFing dataset root (optional)")
    ...
    parser.add_argument("--output", default=str(OUT_DIR / "bio_eval_results.txt"),
 ...
 # 1. Load / generate fingerprint data
    ...
    altered_by_finger: dict[str, list[str]] = {}
    finger_to_subject: dict[str, str]       = {}
    subjects:          dict[str, list[str]] = {}  # fallback mode
    ...
    if args.dataset:
        try:
            ...
            try:
                ...
                print(f"  SOCOFing not found ({exc}). Falling back to synthetic images.")
                subjects = generate_synthetic_subjects(args.subjects)
    else:
        print(f"  Generating {args.subjects} synthetic subjects (4 impressions each) ...")
        subjects = generate_synthetic_subjects(args.subjects)
...
# 5. Sweep thresholds → FAR / FRR
    ...
    if args.plot:
        try:
            ...
            plt.tight_layout()
            plot_path = OUT_DIR / Path(args.output).with_suffix(".png").name
